...
def get_restaurant_details(place_id):
    ...
    params = {
        'place_id': place_id,
        'fields': 'name,formatted_address,formatted_phone_number,rating,user_ratings_total,price_level,opening_hours,website,types,geometry,photos',
        'key': GOOGLE_PLACES_API_KEY,
        'language': 'zh-TW'
    }
    ...
    for restaurant_data in restaurants_data:
        try:
            ...
            if photos:
                ...
                if photo_reference:
                    photo_content = download_restaurant_photo(photo_reference, name)
                    if photo_content:
                        restaurant.image.save(photo_content.name, photo_content, save=True)
                        print(f"📷 已保存 {name} 的照片")
                    # 避免API請求過於頻繁
                    time.sleep(0.2)
...
def main():
    ...
    print(f"   有照片的餐廳: {Restaurant.objects.exclude(image='').count()}")
    
    print(f"\n🎉 現在您可以在搜索功能中找到帶有真實照片的餐廳了！")
