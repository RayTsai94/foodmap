...
def get_ai_response(message, current_page):
    ...
    # 根據當前頁面定制提示詞
    page_contexts = {
        'home': "這是首頁，展示熱門餐廳和分類，您可以瀏覽推薦餐廳、最新動態和美食分類。",
        'restaurant_list': "這是餐廳列表頁面，您可以瀏覽所有餐廳、使用篩選功能、搜尋特定餐廳，並查看餐廳的基本資訊。",
        ...
        'food_analysis': "這是食物分析頁面，提供營養資訊分析、卡路里計算、營養建議等功能。",
        ...
        'ai_food_analysis': "這是AI食物分析頁面，使用人工智慧技術分析食物照片、提供營養估算和建議。",
        'ai_advisor': "這是智能營養顧問頁面，提供個人化的飲食建議、營養規劃和健康指導。",
    ...
    context = page_contexts.get(current_page, "這是NCU食物地圖網站，提供中央大學周邊餐廳資訊、美食推薦、營養分析等功能，協助您探索美食、記錄體驗、了解營養資訊。")
...
7. 標點符號使用全形"""
    ...
    user_prompt = f"""當前頁面：{context}

用戶問題：{message}

請記住：回答必須使用繁體中文，並符合台灣的用語習慣。"""
