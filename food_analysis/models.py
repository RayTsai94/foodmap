...
class UserFoodRecord(models.Model):
    ...
    protein = models.FloatField(help_text='以克為單位', null=True, blank=True)
    carbs = models.FloatField(help_text='以克為單位', null=True, blank=True)
    fat = models.FloatField(help_text='以克為單位', null=True, blank=True)
    fiber = models.FloatField(help_text='以克為單位', null=True, blank=True)
    sugar = models.FloatField(help_text='以克為單位', null=True, blank=True)
    sodium = models.FloatField(help_text='以毫克為單位', null=True, blank=True)
    image = models.ImageField(upload_to='user_food_images/', null=True, blank=True)
    date_consumed = models.DateTimeField(default=timezone.now)
    
    def __str__(self):
        return f"{self.user.username} - {self.name} ({self.date_consumed.strftime('%Y-%m-%d')})"

class UserFoodIngredient(models.Model):
    food_record = models.ForeignKey(UserFoodRecord, on_delete=models.CASCADE, related_name='ingredients')
    ingredient = models.ForeignKey(Ingredient, on_delete=models.CASCADE, related_name='user_food_records')
    amount = models.CharField(max_length=50, blank=True)
    
    def __str__(self):
        return f"{self.ingredient.name} in {self.food_record.name}"
